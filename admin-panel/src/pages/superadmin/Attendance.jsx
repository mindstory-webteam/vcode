...
export default function SuperAdminAttendance() {
  ...
  const submitForm = async (e) => {
    ...
    setBusy(true);
    try {
      await markAttendanceAdmin(studentId, form);
  ...
  const handleDelete = async (record) => {
    ...
    try {
      await deleteAttendanceAdmin(studentId, record._id);
  ...
  const handleBulkFileChange = async (e) => {
    ...
    setBulkBusy(true);
    try {
      const { data } = await bulkUploadAttendanceAdmin(studentId, file);
  ...
  const handleExport = async () => {
    ...
    setExportBusy(true);
    try {
      await exportAttendanceAdmin(studentId, report?.student?.name || 'student');
      ...
      <div className="page-header">
        <div>
          <div className="eyebrow">
            <Link to="/superadmin/students" style={{ color: 'inherit' }}>Students</Link> /{' '}
            <Link to={`/superadmin/students/${studentId}`} style={{ color: 'inherit' }}>
