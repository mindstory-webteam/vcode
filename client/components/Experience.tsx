  ...
  return (
    <section ref={scope} className="pb-12 sm:pb-16">
      <div className="w-full max-w-[1600px] mx-auto relative px-4 sm:px-6 md:px-10 lg:px-16 xl:px-[100px]">
        <div data-animate-exp className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden px-5 sm:px-8 md:px-12 lg:px-14 pt-8 sm:pt-10 md:pt-12 pb-8 sm:pb-12 md:pb-14">
          
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 sm:pb-8 border-b border-gray-200 mb-8 sm:mb-10 gap-3 sm:gap-4">
            <div className="flex items-center gap-4">
              <div>
                <p className="font-mono text-[10px] md:text-[11px] font-bold tracking-[0.2em] text-gray-500 uppercase">
                  Section 05
                </p>
                <h2 className="mt-0.5 font-serif text-2xl sm:text-3xl md:text-4xl text-gray-900 leading-tight">
            ...
            {(experience.role || experience.organization) && (
              <div className="font-sans text-xs sm:text-[14px] md:text-[16px] font-medium text-gray-500 sm:mt-4">
                {experience.role}
                {experience.role && experience.organization && " · "}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
            {allStats.map((s) => (
              <div 
                key={s.label} 
                data-exp-card
                className="border border-gray-200 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 flex flex-col justify-between min-h-[140px] sm:min-h-[160px] md:h-[180px] bg-white transition-shadow hover:shadow-md"
              >
                <div className="mb-2 sm:mb-4">
                  {getIconForStat(s.label)}
                </div>
                <div>
                  <h3 className="font-serif text-2xl sm:text-3xl md:text-[40px] text-gray-900 leading-none tracking-tight">
                    <CountUp to={s.value} suffix={s.suffix} />
                  </h3>
                  <p className="text-gray-500 text-xs sm:text-[13px] md:text-[14px] font-medium mt-2 sm:mt-3 leading-snug">
                    {s.label}
                  </p>
                </div>
              </div>
